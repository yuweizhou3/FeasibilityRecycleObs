...
int main()
{
    ...
    // input value of eta for different constraints
    // the values of eta need to be computed as discussed in the paper

    // single system with two thresholds
    eta[0] = 0.1844;
    ...
    for (int l=0; l<NumMacro; l++) {
        ...
        for (int i=0; i<NumSys; i++) {
   		...
   		for (int i=0; i<NumSys; i++) {
            ...
            int D = 1;
            for (int j=0; j<NumConstraint; j++) D *= surviveThreshold[j];

            int fcp_order[D][NumConstraint];
            int counter_d = 0;

            // single constraint
            for (int d1=0; d1<surviveThreshold[0]; d1++) {
                fcp_order[counter_d][0] = d1;
                counter_d += 1;
            }

            // multiple constraints (five constraints)
            /*for (int d1=0; d1<surviveThreshold[0]; d1++) {
                for (int d2=0; d2<surviveThreshold[1]; d2++) {
                    for (int d3=0; d3<surviveThreshold[2]; d3++) {
                        for (int d4=0; d4<surviveThreshold[3]; d4++) {
                            for (int d5=0; d5<surviveThreshold[4]; d5++) {
                                fcp_order[counter_d][0] = d1;
                                fcp_order[counter_d][1] = d2;
                                fcp_order[counter_d][2] = d3;
                                fcp_order[counter_d][3] = d4;
                                fcp_order[counter_d][4] = d5;
                                counter_d += 1;
                            }
                        }
                    }
                ...
                while (surviveConstraint != 0) {
       				...
       				for (int j=0; j<NumConstraint; j++) {
       					...
       					if (ON[i][j] == 1) {
                            ...
                            if (ON[i][j] == 0) surviveConstraint -= 1;
...
double configuration(void) {
    ...
    // Single system
    system_info[0] = 1;
    for (int j=0; j<NumConstraint; j++) epsilon[j] = 0.1;
